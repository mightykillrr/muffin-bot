  ...
  run: async ({ interaction }) => {
    ...
    if (!actionRow) {
      await interaction.followUp({
        content:
          "There are not enough movies to create a movie night! Please add more movies. (min. 4 required)",
      });
      return;
    }
...
const createMovieNightEmbed = (
  ...
  const descStart =
    "Here are movies to choose from for the next Movie Night. Vote for your movie!";
  const descToVote =
  ...
  const embed = new EmbedBuilder()
    .setAuthor({
      iconURL: guild.iconURL({ size: 128 }) as string,
      name: `${guild.name} - Movie Night!`,
    })
    .setDescription(desc)
    .setTimestamp()
    .setColor("Random");
